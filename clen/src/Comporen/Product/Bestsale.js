  ...
  return (
    <div>
      <div className='flex text-[20px] gap-8 mt-4   items-center pl-4  mr-2'>
            {tabs.map((el)=>(
              <span key={el.id} onClick={() => { setActivedtab(el.id) }} className={` rounded-md flex items-center p-2 justify-center cursor-pointer font-semibold capitalize   border-r shadow-md  ${activedtab === el.id ? "text-gray-900" :"text-gray-400"}`}>{el.name}</span>
            ))}
        </div>
