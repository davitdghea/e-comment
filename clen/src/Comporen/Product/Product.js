...
import 'animate.css'
import img from 'Assets/pngtree-hot-sale-tag-red-corner-label-png-image_9159340.png'
...
const Product = ({ productData, isNew, normal, dispatch, navigate, location,pid }) => {
 const {current} = useSelector(state => state.user)  
  ...
  const handleClickOptions = async (flag) =>{
    ...
    if (flag === 'WISHLIST'){
      const response = await apiUpdateWithlist(pid)
      if(response.success){
        dispatch(getCurrent())
        toast.success(response.mes)
      }else toast.error(response.mes)
  ...
  return (
    <div className=' relative w-full text-base px-[10px] mb-4'>
      <div
        ...
        className='w-full border p-[15px] flex flex-col items-center shadow-lg rounded-xl mb-1'
      >
        ...
        <div className='w-full relative '>
          {isShowOption && <div className='absolute z-20 bottom-[-10px] left-0 right-0 flex justify-center animate__animated animate__fadeInUp gap-2'>
            ...
            <span onClick={() => handleClickOptions("WISHLIST")}><SelectOption icon={<FaHeart color={ current?.wishlist?.some((i) => i._id === pid) ? 'red' : "gray"}/>} /></span> 
          </div>}
          <img
            src={productData?.images[0] || "https://th.bing.com/th/id/OIP.CaLENRpDWR6DvqXLUqlrJgAAAA?rs=1&pid=ImgDetMain"}
            alt=""
            className='w-243px h-243px object-cover'
          />
          {!normal && (isNew === 1 ?
            <div className='w-[100px] h-[10px] absolute bg-red left-[-40px] top-[-25px]'>
              <img src={img} alt=''/>
            </div> : <div className='w-[100px] h-[10px] absolute bg-red left-[-40px] top-[-10px]'>
              <img src={img1} alt='' />
            </div>
          )}
