  ...
  return (
    <div>
      <header className='top-0 z-30 right-0 left-0 fixed sm:relative text-3xl font-semibold py-4 bg-gray-100 border-b border-b-blue-200'>
        <p className='ml-[55px] sm:ml-2'>History</p>
      </header>
      <div className='mt-11'>
        <div className='flex justify-end items-center  mt-3'>
          <form className='w-[50%] flex items-center justify-around gap-4'>
            <div className=''>
              <InputFrom
                id='q'
                register={register}
        ...
        {counts !== 0 ? <table className=' w-full'>
          
            <tr className='border-b pb-5 font-bold bg-gray-700 text-white'>
              <th className='text-[12px] sm:text-[15px] text-center py-2'>STT</th>
          ...
          <tbody>
            {orders?.map((el, idx) => (
              <tr key={el._id} className='border-b py-2'>
                ...
                <td className='text-[12px] text-center sm:text-[15px]'>{formatMoney(Math.round(el.total))}</td>
                <td className={`text-[12px] text-center ${(el.status === 'Cancelled' || el.status === 'False' || el.status === 'Refund') && 'text-red-500'} ${(el.status === 'Order' || el.status === 'Available' || el.status === 'Transport') && 'text-yellow-400' } ${(el.status === 'Succeed') && 'text-blue-500'  } sm:text-[15px]`}>{el.status}</td>
                <td className='text-[12px] text-center sm:text-[15px]'>{moment(el.updatedAt).format('DD/MM/YYYY')}</td>
                {(el.status === 'Order' || el.status === 'Available') && <td className='text-[12px] text-center text-red-500 hover:underline sm:text-[15px] cursor-pointer' onClick={() => setHuydon({ oid: el._id, order: el.status, money: el.total })}>Cancel</td>}
              </tr>
            ))}
          </tbody>
      ...
      {deilProduct.length > 0 && <div onClick={() => setDeilProduct([])} className='flex justify-center items-center min-h-screen h-full absolute top-0 bottom-0 left-0 right-0 bg-overlay' >
        <table onClick={ e => e.stopPropagation()} className='bg-slate-200 py-10 rounded-sm'>
          <tr className='font-bold bg-gray-700  border border-blue-300 text-white'>
      ...
      {huydon?.oid && huydon?.order && huydon?.money && <div onClick={() => setHuydon({ oid: null, order: null, money: null })} className='bg-overlay absolute top-0 bottom-0 left-0 right-0 h-full min-h-screen flex justify-center items-center'>
        <div className='w-full max-w-[350px] rounded-md h-full max-h-[250px] bg-white flex flex-col  justify-center items-center'>
          < AiOutlineExclamationCircle size={80} color={'gray'}/>
